 ...
 *
 * Uses cloudscraper to bypass Cloudflare protection & challenges for free.
...
// ── Cloudscraper HTML fetching with HTTP 429 retry ─────────────────────────────
async function fetchDir(path) {
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      const html = await limiter.schedule(() => cloudscraper.get({
        url: BASE + path,
        headers: {
      ...
      if (err.statusCode === 429) {
        ...
        console.warn(`[daher] cloudscraper fetch error on ${path}: ${err.message}`);
        break;
      }
    }
  }
