 ...
 *
 * Enforces strict quality filtering:
 *   1. Returns ONLY 4K streams if 4K is available.
 *   2. If 4K is not available, falls back to returning ONLY 1080p streams.
 *   3. Completely removes 720p, SD, and lower content under all circumstances.
 */
...
function qualityLabel(s) {
  const res = detectResolution(s.name);
...
function formatStream(s, envLabel = 'Render') {
  const p = s.parsed || {};
  const res = qualityLabel(s);
  ...
  let providerTag;
  if (s.provider === 'daher') {
    providerTag = `🎬 ${envLabel} Daher`;
  } else if (s.provider === 'tpb') {
    providerTag = `🏴‍☠️ ${envLabel} TPB`;
  } else if (s.provider === '4khdhub') {
    providerTag = `🇮🇳 ${envLabel} 4K-HDHUB`;
  } else {
    const nameUpper = (s.provider || '1337x').toUpperCase();
    providerTag = `⚡ ${envLabel} ${nameUpper}`;
  }

  const stream = {
    name: `${providerTag} ${res}`,
    title: lines.join('\n')
  };

  if (s.provider === 'daher' || s.provider === '4khdhub') {
    stream.url = s.url;
    if (s.provider === 'daher') {
      const reqHeaders = {
    ...
    if (s.fileIdx != null) stream.fileIdx = s.fileIdx;
...
function sortStreams(streams) {
  ...
  // 1. If 4K streams exist, return ONLY 4K streams
  const fourKStreams = sorted.filter(s => detectResolution(s.name) === 2160);
  if (fourKStreams.length > 0) {
    console.log(`[filter] 4K streams found (${fourKStreams.length}), returning ONLY 4K streams`);
    return fourKStreams;
  }

  // 2. If NO 4K streams exist, return ONLY 1080p streams (720p and SD are completely removed)
  const hdStreams = sorted.filter(s => detectResolution(s.name) === 1080);
  console.log(`[filter] No 4K streams found (${sorted.length}), falling back to 1080p streams (${hdStreams.length})`);
  return hdStreams;
}
